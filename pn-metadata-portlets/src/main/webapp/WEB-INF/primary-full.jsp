...
<%!    String EXT_LINK_ICON = "<img src=\"decorations/images/external.gif\" alt=\"link to external metadata\" />";
    ...
    static String parseCaps(String value) {
        ...
        if (prev > 0) {
            result = result.concat(value.substring(prev));
        }
        if (prev == 0) {
            result = value;
        }
    ...
    <table class="metadata">
        ...
        for (int i = 0; i < fields.length; i++) {
            ...
            if (values != null && values.length > 0 && !"".equals(values[0])) {
                if (values[0].trim().charAt(0) == '|') {
                    values[0] = values[0].trim().substring(1);
                }
        %>
        ...
        <%
              for (int j = 1; j < values.length; j++) {
                  if (values[j].trim().charAt(0) == '|') {
                      values[j] = values[j].trim().substring(1);
                  }
        %>
        <tr><td>
        ...
        %>
        <tr><th class="rowheader">TM Number</th><td><a href="http://www.trismegistos.org/tm/detail.php?quick=<%=tm%>"><%=tm%></a></td></tr>
        <%
        }
        %>
        <%
        ...
        if ("true".equals(request.getParameter("debug"))) {
            for (int i = 0; i < dFields.length; i++) {
                ...
                if (dValues != null && dValues.length > 0 && !"".equals(dValues[0])) {
                    if (dValues[0].trim().charAt(0) == '|') {
                        dValues[0] = dValues[0].trim().substring(1);
                    }
        %>
        ...
        <%
                    for (int j = 1; j < dValues.length; j++) {
                        if (dValues[j].trim().charAt(0) == '|') {
                            dValues[j] = dValues[j].trim().substring(1);
                        }
        %>
        <tr><td>
