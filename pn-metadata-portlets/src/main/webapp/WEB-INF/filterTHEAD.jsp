<%@page language="java" session="false" contentType="text/html" import="org.apache.lucene.search.*,org.apache.lucene.index.*,org.apache.lucene.document.*,util.jsp.el.Functions,java.util.*,javax.portlet.*,info.papyri.navigator.portlet.*" pageEncoding="UTF-8"%>
...
%>                  <thead>
                      <tr>
                      <th class="rowheader" align="left" colspan="10">
                        <div style="float:right">
                        <form action="">
                        <% 
                        boolean showFilter = false;
                        if (renderRequest.getAttribute(MetadataSearchPortlet.PN_PROVENANCE_REC) != null){
                            showFilter = true;
                            Set<String> opts = (Set<String>)renderRequest.getAttribute(MetadataSearchPortlet.PN_PROVENANCE_REC);
                            Iterator<String> recs = opts.iterator();
                            %>
                            <strong>Filter Provenance:</strong><select id="provFilter">
                            <option value="">[Select]</option>
                            <%while (recs.hasNext()){
                                String next = recs.next();
                            %>
                            <option value="<%=next %>"><%=next %></option>
                            <%} %>
                        </select>
                        <% } 
                        if (renderRequest.getAttribute(MetadataSearchPortlet.PN_SERIES_REC) != null){
                            showFilter = true;
                            Set<String> opts = (Set<String>)renderRequest.getAttribute(MetadataSearchPortlet.PN_SERIES_REC);
                            Iterator<String> recs = opts.iterator();
                            %>
                            <strong>Filter Publications:</strong><select id="serFilter">
                            <option value="">[Select]</option>
                            <%while (recs.hasNext()){
                                String next = recs.next();
                            %>
                        ...
                        <% } 
                        if (renderRequest.getAttribute(MetadataSearchPortlet.PN_LANG_REC) != null){
                            showFilter = true;
                            Set<String> opts = (Set<String>)renderRequest.getAttribute(MetadataSearchPortlet.PN_LANG_REC);
                            Iterator<String> recs = opts.iterator();
                            %>
                            <strong>Filter Language:</strong><select id="langFilter">
                            <option value="">[Select]</option>
                            <%while (recs.hasNext()){
                                String next = recs.next();
                            %>
                        ...
                        <% }
                        if (showFilter){ %>
                        <input type="button" class="pn-button" onclick="doFilter(document.getElementById('ui'));document.getElementById('ui').submit();return true;" value="Filter Search" />
                            <%} %>
                        <input type="button" class="pn-button" onclick="document.getElementById('pn-display-mode').value='search';document.getElementById('ui').submit();return false;" value="Revise Search" />
                        </form>
                      </div>
                      </th>
                      </tr>
                      ...
                      No Results (0)
                      <% }else{ %>
                      Results <%=offset+1 %>-<%=offset+Math.min(numPerPage,numResults - offset) %> (of <%=numResults %>)
                      <%} %></div>
