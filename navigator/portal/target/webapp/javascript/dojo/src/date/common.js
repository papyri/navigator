...
dojo.date.setDayOfYear = function(dateObject, dayOfYear){dateObject.setMonth(0);
dateObject.setDate(dayOfYear);
return dateObject;}
dojo.date.getDayOfYear = function(dateObject){var fullYear = dateObject.getFullYear();
...
dojo.date.getOrdinal = function(dateObject){var date = dateObject.getDate();
if(date%100 != 11 && date%10 == 1){ return "st"; }
else if(date%100 != 12 && date%10 == 2){ return "nd"; }
else if(date%100 != 13 && date%10 == 3){ return "rd"; }
else{ return "th"; }}
dojo.date.compareTypes={DATE:1, TIME:2};
dojo.date.compare=function( dateA,  dateB,  options){var dA=dateA;
var dB=dateB||new Date();
var now=new Date();
with(dojo.date.compareTypes){var opt=options||(DATE|TIME);
var d1=new Date(
...
var sum = new Date(dt);
with(dojo.date.dateParts){switch(interv){case YEAR:
...
break;}}
return sum;};
dojo.date.diff = function( dtA,  dtB,  interv){if(typeof dtA == 'number'){dtA = new Date(dtA);}
...
if (dayDiff > 0) {switch (true) {case aDay == 6:
adj = -1;
break;
case aDay == 0:
adj = 0;
