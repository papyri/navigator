...
11. Run `git merge new_curated` then `git merge new_static`. There may be a merge conflict. Run `git commit -a` anyway and save. There aren't any actual conflicting files, just conflicting state, but Git will be able to cope with two files having the same extended (pre-move) history.
12. Run the `translations.sh` script in the `idp.data` repo root and commit the result.
13. Run `python3 apis-translations.py </path/to/APIS> </path/to/Translations>`. Run `git add Translations`. Commit the result.
