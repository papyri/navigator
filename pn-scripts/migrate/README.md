...
2. Run the `DDbDP-curated.sh` script from inside the `idp.data` repo root. Commit the changes.
3. Run the `DCLP-curated.sh` script from inside the `idp.data` repo root. Commit the changes.
4. Run `python3 generate-current.py </path/to/idp.data>`. Git add the DDbDP folder to pick up the new files and commit the result.
5. Run `python3 updateddb.py </path/to/idp.data>`. Commit the changes.
...
8. Run `DDbDP-static.sh` from inside the `idp.data` repo root. Commit the changes.
9. Run `DCLP-static.sh` from inside the `idp.data` repo root. Run `git add Historical`. Commit the changes.
10. Run `python3 generate-historical.py </path/to/idp.data>`. Git add the Historical folder to pick up the new files and commit the result.
...
14. Run the `translations.sh` script from inside the `idp.data` repo root. Run `git add Translations` and commit the result.
