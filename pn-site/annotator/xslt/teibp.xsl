...
<xsl:stylesheet version="1.0"
  ...
  <xsl:param name="rangycore" select="concat($filePrefix, '/annotator/js/rangy/rangy-core.js')"/>
	...
	<xsl:param name="teibpJS" select="concat($filePrefix,'/annotator/js/teibp.js')"/>
	<xsl:param name="theme.default" select="concat($filePrefix,'/annotator/css/teibp.css')"/>
	<xsl:param name="theme.sleepytime" select="concat($filePrefix,'/annotator/css/sleepy.css')"/>
	<xsl:param name="theme.terminal" select="concat($filePrefix,'/annotator/css/terminal.css')"/>
  ...
  <xsl:template match="tei:unclear">
	...
	<xsl:template match="tei:unclear/text()">
		<xsl:call-template name="split-unclear"><xsl:with-param name="str" select="."/></xsl:call-template>
	</xsl:template>

	<xsl:template name="split-unclear">
		<xsl:param name="str"/>
		<unclear><xsl:call-template name="addID"/><xsl:value-of select="substring($str, 1, 1)"/></unclear>
		<xsl:if test="string-length($str) > 1">
			<xsl:call-template name="split-unclear"><xsl:with-param name="str" select="substring($str, 2)"/></xsl:call-template>
		</xsl:if>
	</xsl:template>
