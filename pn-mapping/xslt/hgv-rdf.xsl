...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
    ...
    <xsl:template match="/tei:TEI">
        <xsl:variable name="id">https://papyri.info/hgv/<xsl:value-of select="//tei:publicationStmt/tei:idno[@type='filename']"/>/source</xsl:variable>
        ...
        <xsl:variable name="cite_uri">https://papyri.info/hgv/<xsl:value-of select="$title"/><xsl:if test="$bibl//tei:biblScope[@type='volume']">_<xsl:value-of select="replace(normalize-space($bibl//tei:biblScope[@type='volume']), ' ', '_')"/></xsl:if><xsl:for-each select="$bibl//tei:biblScope[not(@type='volume')]">_<xsl:value-of select="encode-for-uri(normalize-space(.))"/></xsl:for-each></xsl:variable>
        ...
        <rdf:Description rdf:about="{$id}">
          ...
          -->
            <dct:isPartOf>
                <xsl:choose>
                  <xsl:when test="$bibl//tei:biblScope[@type='volume'][not(matches(., '\s+'))]">
                        <rdf:Description rdf:about="https://papyri.info/hgv/{$title}_{normalize-space($bibl//tei:biblScope[@type='volume'])}">
                          ...
                          <rdf:type rdf:resource="http://purl.org/ontology/bibo/Book"/>
                            <dct:isPartOf>
                                <rdf:Description rdf:about="https://papyri.info/hgv/{$title}">
                                  ...
                                  <dct:bibliographicCitation><xsl:value-of select="$bibl/tei:title[@level='s']"/></dct:bibliographicCitation>
                                   <dct:isPartOf rdf:resource="https://papyri.info/hgv"/>
                                </rdf:Description>
                            </dct:isPartOf>
                        </rdf:Description>
                    </xsl:when>
            ...
            <xsl:if test="(//tei:publicationStmt/tei:idno[@type='ddb-hybrid']) and doc-available($ddb-doc-uri)">
                <dct:relation>
                    <rdf:Description>
                    	<xsl:attribute name="rdf:about">https://papyri.info/ddbdp/<xsl:value-of select="replace(normalize-unicode($ddb[1], 'NFD'), '[^.a-z0-9]', '')"/>;<xsl:value-of select="$ddb[2]"/>;<xsl:value-of select="encode-for-uri($ddb[3])"/>/source</xsl:attribute>
          ...
          <xsl:for-each select="//tei:idno[@type = 'TM']">
            ...
            <xsl:for-each select="//tei:text/tei:body/tei:div[@type='figure']//tei:graphic[contains(@url, 'columbia.edu') and contains(@url, 'key')]">
                <dct:relation>
                    <rdf:Description rdf:about="https://papyri.info/apis/{normalize-space(substring-after(@url, 'key='))}/source">
                        <dct:relation rdf:resource="{$id}"/>
                        <dct:relation>
                            <rdf:Description> 
            ...
            <dct:source>
                <rdf:Description rdf:about="https://papyri.info/hgv/{//tei:publicationStmt/tei:idno[@type='filename']}/work">
        ...
        <xsl:for-each select="//tei:msDesc/tei:history/tei:provenance[@type='located']/tei:p/tei:placeName[contains(@ref,'//pleiades')]">
          <rdf:Description>
            <xsl:attribute name="rdf:about">https://papyri.info/current/<xsl:value-of select="//tei:idno[@type='filename']"/>/original</xsl:attribute>
            <xsl:variable name="label" select="string(.)"/>
            <xsl:for-each select="tokenize(@ref,'\s')">
              <xsl:if test="starts-with(.,'http://pleiades') or starts-with(., 'https://pleiades')">
                <lawd:foundAt>
