package info.papyri.data.provenance;
...
public class ProvenanceControl {
    ...
    public static String match(String provenance){
        ...
        for (int i=0;i< PROVENANCE.length; i++){
            int d = info.papyri.util.EditDistance.distance(source, PROVENANCE[i]);
    ...
    public static String matchShingled(String provenance){
        if (provenance == null) return null;
        provenance = provenance.toLowerCase().trim();
        String match = null;
        match = CANON.get(provenance);
        ...
        char [] source = provenance.toLowerCase().trim().toCharArray();
        for (int i=0;i< PROVENANCE.length; i++){
            int d = info.papyri.util.EditDistance.shingleDistance(source, PROVENANCE[i]);
            if (d == 0) return CANON.get(new String(PROVENANCE[i]));
            if (d < distance && d <= min){
                result = i;
                distance = d;
