...
//    private ArrayList<String> lines = null;
    ...
    public Iterator<String> getLines(){
        ...
        for (int i=0;i<lines.length;i++){
            if (lines[i].endsWith("-") && i < (lines.length - 1)){
                ...
                if (secondPartOffset == -1) secondPartOffset = lines[i+1].length();
    ...
    public void characters(char[] cdata, int start, int length) throws SAXException {
		if (collect.peek().booleanValue()){
            ...
            else text.append(cdata,start,length);
//			line.append(cdata,start,length);
    ...
    public static final String [] getNameParts(String id, String n){
        ...
        result[COLL_IX] = nFrags[0];
        try{
        if(nFrags.length == 3){ 
            result[DOC_IX] =nFrags[2].replace(',','-').replace('/', '_');
            if( nFrags[1] != null && !nFrags[1].equals("")){ 
                result[VOL_IX] = nFrags[1];
                result[COLL_IX] = id.substring(0,id.indexOf("." + result[VOL_IX]));
            }
            else{
