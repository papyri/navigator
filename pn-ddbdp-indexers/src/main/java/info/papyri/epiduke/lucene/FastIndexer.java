...
//        plain.add(new Field(Indexer.LEMMA_TERM,WORD_SPAN_ANALYZER.tokenStream(null, new StringReader(""))));
        ...
        if (file.isFile() && fname.endsWith(".xml")){
            ...
            files++;
            System.out.print('.');
            if(files%250==0)System.out.println(files);
...
//                plain.getField(Indexer.LEMMA_TERM).setValue(template.getField(Indexer.LEMMA_TERM).tokenStreamValue());
                ...
                if(this.coreMetadata != null){
                    ...
                    if(!coreId.startsWith(NamespacePrefixes.DDBDP)) throw new RuntimeException("Malformed id: " + coreId);
                    ...
                    while(iterator.hasNext()){
                        ...
                        if(dates != null){
                            for(String date:dates)metadata.add(new Field(CoreMetadataFields.DATE1_D,date,Field.Store.YES, Field.Index.NO));
                        }
                        ...
                        if(places != null){
                            for(String place:places)metadata.add(new Field(CoreMetadataFields.PROVENANCE_NOTE,place,Field.Store.YES, Field.Index.TOKENIZED));
                        }
                        String title = mDoc.get(CoreMetadataFields.TITLE);
                        if(title != null) metadata.add(new Field(CoreMetadataFields.TITLE,title,Field.Store.YES,Field.Index.NO));
        ...
        if (file.isDirectory() && recur){
            for (String child:file.list()){
                index(new File(file,child),recur,template);
            }
        }
    }
