 ...
 * <pre>      t1 .. t2 t3</pre>
 */
class SequenceSpans implements Spans {
//  private final int allowedSlop;
  ...
  private SpanSequenceQuery query;

  public SequenceSpans(info.papyri.epiduke.lucene.spans.SpanSequenceQuery spanNearQuery, IndexReader reader)
  throws IOException {
...
//    allowedSlop = spanNearQuery.getSlop();
    ...
    for (int i = 0; i < clauses.length; i++) {
      ...
      subSpansByDoc[i] = subSpans[i]; // used in toSameDoc() for sorting
    }
  ...
  private boolean toSameDoc() throws IOException {
    ...
    for (int i = 0; i < subSpansByDoc.length; i++) {
      assert (subSpansByDoc[i].doc() == maxDoc)
             : " SequenceSpans.toSameDoc() spans " + subSpansByDoc[0]
  ...
  private boolean nextSequence() throws IOException {

      matchStart = subSpans[0].start();
      matchEnd = subSpans[0].end();

      for (int i = 1; inSameDoc && (i < subSpans.length); i++) {
          while (inSameDoc &&  (subSpans[i].start() < matchEnd)) {
              if (! subSpans[i].next()) {
                  inSameDoc = false;
                  more = false;
                  return false;
              }
              inSameDoc = (matchDoc == subSpans[i].doc());
          }
          if(!inSameDoc){
          ...
          if(matchEnd != subSpans[i].start()){ // advance starting span and re-enter
              more = subSpans[0].next();
              inSameDoc = (more && (matchDoc == subSpans[0].doc()));
              return false;
          }
          matchEnd = subSpans[i].end();
      }

      //here we have matched all the way through
      more = subSpans[0].next();
      inSameDoc = (more && (matchDoc == subSpans[0].doc()));
      return true;
  }

  private boolean nextSequenceRev() throws IOException {

      int start = subSpans.length  - 1;
      matchStart = subSpans[start].start();
      matchEnd = subSpans[start].end();

      for (int i = subSpans.length - 2; inSameDoc && (i >= 0); i--) {
          while (inSameDoc &&  (subSpans[i].end() < matchStart)) {
              if (! subSpans[i].next()) {
                  inSameDoc = false;
                  more = false;
                  return false;
          ...
          if(matchStart != subSpans[i].end()){
              if (! subSpans[start].next()) { // since we'll start over from 0, should we shortcut to sub[0] rather than prev?
                  inSameDoc = false;
                  more = false;
              } else if (matchDoc != subSpans[start].doc()) {
                  inSameDoc = false;
              }
              return false;
          }
          matchStart = subSpans[i].start();
      }

      //here we have matched all the way through
      more = subSpans[start].next();
      return true;
  }
  ...
  private boolean shrinkToAfterShortestMatch() throws IOException {
      ...
      return matchSlop == 0; // ordered and allowed slop
