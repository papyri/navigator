...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <xsl:template match="t:supplied[@reason='lost']" mode="reg">
      <xsl:param name="location"/>
      <xsl:if
         test="($leiden-style = 'ddbdp' or $leiden-style = 'sammelbuch') and child::t:*[1][local-name() = 'milestone'][@rend = 'paragraphos']">
         <br/>
      </xsl:if>
      <xsl:choose>
         ...
         <xsl:otherwise>
            ...
            <xsl:if test="EDF:f-wwrap(.) = true()">
               ...
               <xsl:if
                  test="(not($leiden-style='ddbdp' and (ancestor::t:*[local-name()=('reg','corr','rdg') or self::t:del[parent::t:subst]]))) and (not($location = 'apparatus'))">
                  <xsl:text>-</xsl:text>
               </xsl:if>
            </xsl:if>
            <!-- Found in tpl-reasonlost.xsl -->
