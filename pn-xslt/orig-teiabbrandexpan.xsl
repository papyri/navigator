...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="orig-edition-wrapper//t:ex">
        <xsl:if test="preceding-sibling::node() or preceding-sibling::text()"><xsl:value-of select="$abbreviation-marker"></xsl:value-of></xsl:if>
        <xsl:apply-templates></xsl:apply-templates>
        <xsl:if test="following-sibling::node() or following-sibling::text()"><xsl:value-of select="$abbreviation-marker"></xsl:value-of></xsl:if>
    </xsl:template>
